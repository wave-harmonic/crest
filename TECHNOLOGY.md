...
This data is populated by the AnimatedWavesCamX cameras, where X is the LOD index (these are created on startup and can be inspected at runtime). The culling is set to the layer *LODDataAnimatedWaves*, so any geometry in this layer will modify the ocean shape. An example of this is visible in the *boat.unity* scene, where a smoothstep bump is added to the water shape. This is an efficient way to generate dynamic shape.
...
The Foam LOD Data is simple type of simulation for foam on the surface. Foam is generated by choppy water (specifically when the surface is *pinched*), or can be added assigning the *LodDataFoam* to your geometry/shader.
