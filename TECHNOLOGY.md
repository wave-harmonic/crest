...
The backbone of Crest is an efficient LOD'd representation for data that drives the rendering, such as surface shape/displacements, foam values, shadowing data, water depth, and others. This data is stored in a multi-resolution format - cascaded textures that are centered at the viewer.

The surface shape is given by *displacement textures* which describe the surface shape. A top down view of this shape LOD data looks as follows:
...
In this example the largest LOD covers a large area (4km squared), while maintaining high detail around the viewer at the center due to the cascaded LODs. Each LOD texture has the same dimensions, and these textures are visualised in the Debug GUI on the right hand side of the screen:
