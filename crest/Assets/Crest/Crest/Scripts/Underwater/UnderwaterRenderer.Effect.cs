...
namespace Crest
{
    ...
    public partial class UnderwaterRenderer
    {
        ...
        static readonly int sp_CrestInvViewProjection = Shader.PropertyToID("_CrestInvViewProjection");
        static readonly int sp_CrestInvViewProjectionRight = Shader.PropertyToID("_CrestInvViewProjectionRight");
        static readonly int sp_CrestAmbientLighting = Shader.PropertyToID("_CrestAmbientLighting");
        static readonly int sp_CrestHorizonNormal = Shader.PropertyToID("_CrestHorizonNormal");
        ...
        internal static void SetUnderwaterGlobals(CommandBuffer commandBuffer, Camera camera, UnderwaterSphericalHarmonicsData sphericalHarmonicsData)
        {
            // Have to set these explicitly as the built-in transforms aren't in world-space for the blit function
            if (XRHelpers.IsSinglePass)
            {
                // NOTE: Not needed for HDRP.
                commandBuffer.SetGlobalMatrix(sp_CrestInvViewProjection, (GL.GetGPUProjectionMatrix(XRHelpers.LeftEyeProjectionMatrix, false) * XRHelpers.LeftEyeViewMatrix).inverse);
                commandBuffer.SetGlobalMatrix(sp_CrestInvViewProjectionRight, (GL.GetGPUProjectionMatrix(XRHelpers.RightEyeProjectionMatrix, false) * XRHelpers.RightEyeViewMatrix).inverse);
            }
            else
            {
                // NOTE: Not needed for HDRP.
            ...
            // Project ocean normal onto camera plane.
            {
                ...
                commandBuffer.SetGlobalVector(sp_CrestHorizonNormal, projectedNormal);
            }
