...
{
	half ft = lerp(
		tex2D(_FoamTexture, (1.25*i_worldXZUndisplaced + _CrestTime / 10.) / (4.*_CascadeData[_LD_SliceIndex]._texelWidth*_FoamScale)).r,
		tex2D(_FoamTexture, (1.25*i_worldXZUndisplaced + _CrestTime / 10.) / (4.*_CascadeData[_LD_SliceIndex + 1]._texelWidth*_FoamScale)).r,
		lodVal);
...
half BubbleFoamTexture(float2 i_worldXZ, float2 i_worldXZUndisplaced, half3 i_n, half3 i_view, half lodVal)
{
	...
	half ft = lerp(
		tex2Dlod(_FoamTexture, float4((0.74 * foamUVBubbles + parallaxOffset) / (4.0*_CascadeData[_LD_SliceIndex]._texelWidth), 0., 3.)).r,
		tex2Dlod(_FoamTexture, float4((0.74 * foamUVBubbles + parallaxOffset) / (4.0*_CascadeData[_LD_SliceIndex + 1]._texelWidth), 0., 3.)).r,
		lodVal);
