...
{
	half ft = lerp(
		tex2D(_FoamTexture, (1.25*i_worldXZUndisplaced + _CrestTime / 10.) / (4.*_LD_Params[_LD_SLICE_Index_ThisLod].x*_FoamScale)).r,
		tex2D(_FoamTexture, (1.25*i_worldXZUndisplaced + _CrestTime / 10.) / (4.*_LD_Params[_LD_SLICE_Index_ThisLod + 1].x*_FoamScale)).r,
		lodVal);
