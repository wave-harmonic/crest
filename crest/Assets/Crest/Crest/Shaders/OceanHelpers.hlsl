...
void ComputePositionDisplacement(inout float3 io_positionWS, in const float i_lodAlpha)
{
	...
	{
		SampleDisplacements(_LD_TexArray_AnimatedWaves, WorldToUV(worldXZ, _LD_Pos_Scale[_LD_SliceIndex], _LD_Params[_LD_SliceIndex], _LD_SliceIndex), wt_smallerLod, io_positionWS, dummy);
	}
	if (wt_biggerLod > 0.001)
	{
		const uint si = _LD_SliceIndex + 1;
		SampleDisplacements(_LD_TexArray_AnimatedWaves, WorldToUV(worldXZ, _LD_Pos_Scale[si], _LD_Params[si], si), wt_biggerLod, io_positionWS, dummy);
	}
}
