...
#define LOD_DATA(FRAMENUM) \
	...
	uniform float4 _LD_Params_##FRAMENUM; \
	uniform float3 _LD_Pos_Scale_##FRAMENUM;

uniform float _LD_SLICE_Index_0;
...
{
	return float3(i_uv, _LD_SLICE_Index_0);
}

float3 ADD_SLICE_1_TO_UV(in float2 i_uv)
{
	return float3(i_uv, _LD_SLICE_Index_0 + 1);
}
...
void SampleFoam(in Texture2DArray i_oceanFoamSampler, float3 i_uv_slice, in float i_wt, inout half io_foam)
{
	io_foam += i_wt * i_oceanFoamSampler.SampleLevel(LODData_linear_clamp_sampler, i_uv_slice, float2(0, 0)).x;
}

void SampleFlow(in Texture2DArray i_oceanFlowSampler, float3 i_uv_slice, in float i_wt, inout half2 io_flow)
{
