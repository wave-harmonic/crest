...
void SnapAndTransitionVertLayout(in const float i_meshScaleAlpha, in const CascadeParams i_cascadeData0, in const float i_geometryGridSize, inout float3 io_worldPos, out float o_lodAlpha)
{
	...
	float2 fractional = UNITY_MATRIX_M._m03_m23 - (round( UNITY_MATRIX_M._m03_m23 / GRID_SIZE_2 - 0.5 )) * GRID_SIZE_2;
	io_worldPos.xz -= fractional;
	//io_worldPos.xz -= frac( io_worldPos.xz / GRID_SIZE_2) * GRID_SIZE_2; // caution - sign of frac might change in non-hlsl shaders
	//io_worldPos.xz -= UNITY_MATRIX_M._m03_m23 - round( UNITY_MATRIX_M._m03_m23 / GRID_SIZE_2 - .5*i_geometryGridSize ) * GRID_SIZE_2;
