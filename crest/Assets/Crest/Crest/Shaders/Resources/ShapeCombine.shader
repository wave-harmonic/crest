...
Shader "Hidden/Crest/Simulation/Combine Animated Wave LODs"
{
	SubShader
	{
		...
		Pass
		{
			...
			{
				const float period = 3.0 * _CascadeDataTgt[_LD_SliceIndex]._texelWidth;
			...
			half4 Frag(Varyings input) : SV_Target
			{
				...
				const float2 worldPosXZ = UVToWorld(input.uv, _LD_SliceIndex, _CascadeDataTgt[_LD_SliceIndex]);

				// sample the shape 1 texture at this world pos
				const uint si = _LD_SliceIndex + 1;
				const float3 uv_nextLod = WorldToUV(worldPosXZ, _CascadeDataTgt[si], si);
				...
				const float3 uv_thisLod_flow_0 = WorldToUV(worldPosXZ - offsets[0] * flow, _CascadeDataTgt[_LD_SliceIndex], _LD_SliceIndex);
				const float3 uv_thisLod_flow_1 = WorldToUV(worldPosXZ - offsets[1] * flow, _CascadeDataTgt[_LD_SliceIndex], _LD_SliceIndex);
...
#if CREST_DYNAMIC_WAVE_SIM_ON_INTERNAL
				{
					...
					const float2 invRes = float2(_CascadeDataTgt[_LD_SliceIndex]._oneOverTextureRes, 0.0);
					...
					const float wavelength_mid = 2.0 * _CascadeDataTgt[_LD_SliceIndex]._texelWidth * 1.5;
					const float wavevector = 2.0 * 3.14159 / wavelength_mid;
					const float2 dydx = (float2(waveSimY_px, waveSimY_pz) - float2(waveSimY_nx, waveSimY_nz)) / (2.0 * _CascadeDataTgt[_LD_SliceIndex]._texelWidth);
					float2 dispXZ = _HorizDisplace * dydx / wavevector;

					const float maxDisp = _CascadeDataTgt[_LD_SliceIndex]._texelWidth * _DisplaceClamp;
