...
Shader "Hidden/Crest/Underwater/Apply Underwater Fog"
{
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 Frag (Varyings input) : SV_Target
			{
				...
				GetOceanSurfaceAndUnderwaterData(rawOceanDepth, mask, isBelowHorizon, rawDepth, isOceanSurface, isUnderwater, sceneZ, 0.0);
					...
					return fixed4(DebugRenderOceanMask(isOceanSurface, isUnderwater, mask, 0.0).rgb, alpha);
				#endif // _DEBUG_VIEW_OCEAN_MASK

				half3 scatterColor = 0.0;
				...
				if (isUnderwater)
				{
					...
					scatterColor = GetScatterColor(_AmbientLighting, lightCol, lightDir, view);
				}
				else
				{
					alpha = 0;
				...
				float depthFogDensity = (_DepthFogDensity.x + _DepthFogDensity.y + _DepthFogDensity.z) / 3.0;
				// return fixed4(scatterColor, saturate(1.0 - exp(-depthFogDensity * sceneZ)) * alpha);
				return fixed4(scatterColor, saturate(1.0 - exp(-depthFogDensity * sceneZ)) * alpha);
			}
			ENDHLSL
		}
