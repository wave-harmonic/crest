...
)
{
	...
	if (isUnderwater)
	{
		mask = PostProcessHandleOccluderMask(positionNDC, mask, rawOceanDepth);
		isUnderwater = mask == UNDERWATER_MASK_BELOW_SURFACE;
	}
	...
	if (isOceanSurface)
	{
		...
		// TODO: Need to make one for occluders...
	...
	const float rawOccluderDepth = tex2D(_CrestOceanOccluderMaskDepthTexture, positionNDC).x;

	if (rawDepth < rawOccluderDepth)
	{
		isOceanSurface = true;
		rawDepth = rawOccluderDepth;
	}
