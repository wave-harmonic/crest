...
public class WaveParticlesSystemOld
{
    ...
    ///
    /// If there is already a pointer to a WaveParticleOld in _subdivisionEvents at
    ...
    private int[] _subdivisionEvents = new int[WaveParticleOld.FRAME_CYCLE_LENGTH];

    private const string PENDING_PARTICLES_HEAD = "pendingParticlesHead";
    private int _pendingParticlesHead = 0;
    private WaveParticleOld[] _pendingParticles;
    ...
    public void Initialise(int numParticles, float waveParticleKillThreshold)
    {
        ...
        _waveParticlesBuffer = new ComputeBuffer((int)_particleContainerSize, WaveParticleOld.STRIDE);
        ...
        _pendingParticles = new WaveParticleOld[numParticles / 10];
    }
    ...
    public void addParticle(WaveParticleOld particle)

    {
        ///
        /// Calculate when WaveParticleOld is due to subdivide
        ///
        int subdivisionFrame;
        {
            ...
            {
                subdivisionFrame = particle.startingFrame + WaveParticleOld.FRAME_CYCLE_LENGTH;
            }
            else
            {
                float timeToSubdivision = WaveParticleOld.RADIUS * 0.5f / (WaveParticleOld.PARTICLE_SPEED * particle.dispersionAngle);
                int framesToSubdivision = (Mathf.RoundToInt(timeToSubdivision / Time.fixedDeltaTime));
                if (framesToSubdivision > WaveParticleOld.FRAME_CYCLE_LENGTH)
                {
                    framesToSubdivision = WaveParticleOld.FRAME_CYCLE_LENGTH;
                }
                subdivisionFrame = (framesToSubdivision + particle.startingFrame) % WaveParticleOld.FRAME_CYCLE_LENGTH;
            }
        }
    ...
    public void commitParticles()
    {
        ...
        ///
        ComputeBuffer pendingParticlesBuffer = new ComputeBuffer(_pendingParticlesHead + 1, WaveParticleOld.STRIDE);
    ...
    public void calculateSubdivisions(int currentFrame)
    {
        ...
        ComputeBuffer particlesToAddBuffer = new ComputeBuffer(numSubdivisions * 3, WaveParticleOld.STRIDE, ComputeBufferType.Append);
        ...
        _gpuSubdivideParticles.SetInt(FRAME_CYCLE_LENGTH, WaveParticleOld.FRAME_CYCLE_LENGTH);
        _gpuSubdivideParticles.SetInt(NUM_PARTICLES_TO_ADD, numParticlesToAdd);
        _gpuSubdivideParticles.SetFloat(WAVE_PARTICLE_RADIUS, WaveParticleOld.RADIUS);
        _gpuSubdivideParticles.SetFloat(FIXED_DELTA_TIME, Time.fixedDeltaTime);
        _gpuSubdivideParticles.SetFloat(PARTICLE_SPEED, WaveParticleOld.PARTICLE_SPEED);
    ...
    public void splatParticles(int currentFrame, ref ExtendedHeightFieldOld pointMap)
    {
        ...
        _gpuSplatParticles.SetFloat(PARTICLE_SPEED, WaveParticleOld.PARTICLE_SPEED);
    ...
    public void splatParticlesModulus(int currentFrame, ref ExtendedHeightFieldOld pointMap)
    {
