	...
	Category
	{
		...
		SubShader {
			Pass {
				...
				uniform half _FlowFadeRate;
				uniform half _WaveFlowStrength;
				uniform half _WaveFlowCoverage;
				uniform half _ShorelineFlowMaxDepth;
				uniform half _ShorelineFlowStrength;
				...
				half frag(v2f i) : SV_Target
				{
					...
					half foam = tex2Dlod(_LD_Sampler_Flow_0, uv_lastframe).x;
					...
					// fade
					foam *= max(0.0, 1.0 - _FlowFadeRate * _SimDeltaTime);
					...
					foam += 5. * _SimDeltaTime * _WaveFlowStrength * saturate(_WaveFlowCoverage - det);

					// add foam in shallow water
					float signedOceanDepth = tex2Dlod(_LD_Sampler_SeaFloorDepth_1, uv).x + DEPTH_BIAS + disp.y;
					foam += _ShorelineFlowStrength * _SimDeltaTime * saturate(1. - signedOceanDepth / _ShorelineFlowMaxDepth);
