...
// solve 2D shallow water equations
Shader "Hidden/Ocean/Simulation/Update Dynamic Waves SWE"
{
	SubShader
	{
		Pass
		{
			...
			{
				half2 flow = tex2Dlod(_LD_Sampler_Flow_1, float4(input.uv, 0, 0));
				float2 uv_lastframe = LD_0_WorldToUV(input.worldPosXZ - (_SimDeltaTime * flow));
				float4 uv_lastframe4 = float4(uv_lastframe, 0., 0.);

				half2 Ht_ftm = tex2Dlod(_LD_Sampler_DynamicWavesSWE0_0, uv_lastframe4);

				//float2 u = input.uv_uvprev.xy;
				//float Hx = 
				//float2 gradH;
				//Ht = dot(-u, )

				half2 ft_ftm = tex2Dlod(_LD_Sampler_DynamicWavesSWE0_0, uv_lastframe4);
				...
				float fxm = tex2Dlod(_LD_Sampler_DynamicWavesSWE0_0, uv_lastframe4 - e * X).x; // x minus
				float fym = tex2Dlod(_LD_Sampler_DynamicWavesSWE0_0, uv_lastframe4 - e * Y).x; // y minus
				float fxp = tex2Dlod(_LD_Sampler_DynamicWavesSWE0_0, uv_lastframe4 + e * X).x; // x plus
				float fyp = tex2Dlod(_LD_Sampler_DynamicWavesSWE0_0, uv_lastframe4 + e * Y).x; // y plus
				...
				float waterSignedDepth = DEPTH_BASELINE - tex2D(_LD_Sampler_SeaFloorDepth_1, float4(input.uv, 0., 0.)).x;
				float depthMul = 1. - (1. - saturate(2.0 * waterSignedDepth / wavelength)) * dt * 2.;
				ftp *= depthMul;
				ft *= depthMul;

				SimOutput o = (SimOutput)0;
				o.h_hprev = half2(ftp, ft);
				o.uv_uvprev.g = 1.;
				return o;
			}
